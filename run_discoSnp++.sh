...
if [ ! -e $h5prefix.h5 ]; then
       ...
       graphCmd="${dbgh5_bin} -in ${read_sets}_${kissprefix}_removemeplease -out $h5prefix -kmer-size $k -abundance-min ${c_dbgh5} -abundance-max $C -solidity-kind one ${option_cores_gatb} -no-mphf -verbose $verbose"
       echo ${graphCmd}
       ${graphCmd}
