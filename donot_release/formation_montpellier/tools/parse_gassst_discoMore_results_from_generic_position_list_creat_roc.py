        ...
        if line.startswith(typePolymorphism):
            ...
            if predicted_pol_id in list_predicted_polymorphism:
                ...
                if typePolymorphism == "INDEL":
                    ...
                    forward)
            if localNbMapped>0:
                list_predicted_polymorphism[predicted_pol_id] = localNbMapped # We mapped the polymorphism(s) corresponding to this predicted id. `
...
#                         MAIN
####################################################################
def doAll(discoResFile, logFile, gassstResFile, polymorphism, threshold, verbose,generic,roc):
    ...
    if roc:
        ...
        sys.stdout = f
        parse_gassst_common.printRoc(discoResFile, list_reference, list_predicted, threshold, polymorphism)
        f.close()
        sys.stdout = prev_out
    if verbose:
...
def main():
    ...
    doAll(discoFile, logFile, gassstFile, "SNP", threshold, verbose,generic,roc)
    doAll(discoFile, logFile, gassstFile, "INDEL", threshold, verbose,generic,roc)
