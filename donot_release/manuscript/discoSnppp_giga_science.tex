...
%CL plutot new software design ? du point de vue algo ce n'est pas different, non ?
...
%CL : enleve la partie classification et ranking ?? are analyzed to be classified and ranked with respect to several criteria, and output. %and output ?
...
%CL je mettrais ces paragraphes plutôt dans la partie suivante, ce n'est plus vraiment de l'intro, ça devient technique
%CL essayer de raccourcir pour pas que discoSNP prenne plus de place que discoSnp++
...
%CL plus de details algo ici ? comment on gere les différents types de variants, gestion des branchements
%CL pour l'affichage : que ca prenne un plus de place que la description de discoSnp++, mais aussi parce que c'est interessant :-) 
...
%CL je pense que c'est important de détailler quelles tailles d'indels aussi pour se positionner pour les utilisateurs, il y a clairement une distinction entre 3 tailles d'indels pour les methodes par mapping : les petits = ceux qui peuvent être contenus dans un alignement de mapper (GATK), les moyens : ceux qui sont détectables par les reads pairés/split-read (< taille d'insert), les grands (plus grand que la taille d'insert) détectables par assemblage ou read-depth. (rq : la distinction moyen/grand dépent si on parle d'insertion ou de délétion). En vrai, nous on se place plutôt dans la catégorie "petit" (on ne vise pas les structural variants), mais on peut surement detecter des indels plus grands que GATK, etc qui sont limités par les mappers.
...
%CL TODO ajout des ref
%CL bientot rajouter MindTheGap, BGREAT ?
...
\section*{Results}
%CL suggestion pour que ca ressemble pas trop a la publi NAR : changer les couleurs des figures, changer les figures (mais je n'ai pas d'idées comment...)
We propose results both on synthetic and on real datasets. Synthetic datasets offer a way to exactly compute the precision and the recall of \discopp and of state-of-the-art methods. Real datasets enable to assess the %CL phrase pas finie
...
%CL je pense que c'est important de détailler quelles tailles d'indels, idem pour se positionner par rapport aux autres methodes : si les indels simulés sont trop grands, ca veut dire qu'il faut se comparer a d'autres concurrents que GATK.
...
%CL si on a implementé le seuil auto par jeu de donnees, pourrait faire une expe avec des couvertures differentes dans chaque jeu : on serait peut-être bien meilleurs que les autres ?? (on laisserait celle-là en mat. supp pour montrer que meme sans ces variations on est meilleur)
...
%PP TODO: changer ce paragraphe avec les nouveaux résultats disco. 
...
%CL : idem pour precision/recall : nos valeurs de precision devraient augmenter si on prend que les variants avec le filter PASS dans le VCF
...
%CL je trouverai interessant d'avoir des resus sur genome distant... mais peut-etre que c'est dangereux, on pourrait nous demander de comparer avec GATK sur genome de ref
...
%CL TODO on devrait le vérifier avec les résultats de mapping -> mappés multiplement ? 
...
% \section*{Conclusion}
% TODO
%CL en conclusion pourrait dire que ce n'est plus (ou encore moins qu'avant) limité aux projets sans genome de ref, puisqu'avec le vcf on se retrouve avec le meme fichier de sortie... mais encore une fois dangereux...
