...
We propose \discopp, the successor of \disco, which benefits from a new software 
design that reduces time and memory consumption, and from a new algorithmic design that detects all kinds of SNP and small indels, adds genotype information and outputs a VCF file. Results show that \discopp performs better than state-of-the-art methods in terms of both quality and computational resources. 
...
% Before to present improvements made in \discopp we briefly recall the main algorithmic ideas of \disco.

%\subsubsection*{\discopp} % (fold)
...
%PP : yep todo. Il faudrait que l'on fasse des tests.
...
%PP: pour l'instant j'ai changé les couleurs.
...
%CL si on a implementé le seuil auto par jeu de donnees, pourrait faire une expe avec des couvertures differentes dans chaque jeu : on serait peut-être bien meilleurs que les autres ?? 
%PP: avec la correction de la validation des SNPs détectés plusieurs fois, on est déjà meilleurs que les autres. Ceci dit si on pouvait faire encore mieux faut pas s'en priver. 
...
%SIZE INDELS
%grep INDEL ref_human | cut -d " " -f 4,5 > afac
%for i in  `cat afac`; do echo $i | wc -m ; done > afac2
%R: a=read.table("afac2")
% summary(a[a$V1>2,1]-2) --> -2 pour contrer l'effet wc et car les mot de taille n sont des indel de taille n-1
