...
def storeThePolymorphismPositions(simulatorLogFile, typePolymorphism):
    ...
    list_simulated_polymorphism={}
    while 1:
        line=filin.readline()
        if not line: break
        if line.startswith(">"+typePolymorphism):
            pos=int(line.split("|")[2]) #>SNP_2999|lower|4637891|C/T
            if pos in list_simulated_polymorphism:
                print "Warning, more than one "+typePolymorphism+" was simulated position "+str(pos)
            else:
                list_simulated_polymorphism[pos]=False # Will be set to true if this SNP is detected. 
        filin.readline() # read sequence upper, we don't care
        filin.readline() # read comment lower, we don't care
        filin.readline() # read sequence lower, we don't care
...
def findNbMachedPolymorphism(list_simulated_polymorphism, start, stop):
    nbMapped=0 # True positive prediction 
    for pos in range(start, stop+1):
        if pos in list_simulated_polymorphism and list_simulated_polymorphism[pos]==False:
            list_simulated_polymorphism[pos]=True
            nbMapped+=1
...
def checkMappedPaths(gassstOutFile,typePolymorphism, list_simulated_polymorphism, list_predicted_polymorphism,predicted_polymorphism_both_mapped, threshold):
    filin = open(gassstOutFile, 'r')
    for line in filin.readlines(): # SNP_lower_path_95|high|nb_pol_1|C1_41|C2_0|rank_1.00000 ...
        if line.startswith(typePolymorphism):
            ...
            if predicted_pol_id in list_predicted_polymorphism: #and not predicted_pol_id in predicted_polymorphism_both_mapped:
                localNbMapped = findNbMachedPolymorphism(list_simulated_polymorphism,matching_position-1,matching_position+span_match-1)
            if localNbMapped>0:
...
def print_results(nb_predicted, list_simulated, polymorphism, threshold):
    ...
    print len(list_simulated), polymorphism,"were simulated.\t Among them", nbTP, "are correctly predicted"
...
def doAll(discoResFile, logFile, gassstResFile, polymorphism, threshold, verbose):
    ...
    nb_predicted=res[1]
    list_simulated=storeThePolymorphismPositions(logFile, polymorphism)
    predicted_polymorphism_both_mapped = checkDoubleMappedPaths(gassstResFile,polymorphism)
    checkMappedPaths(gassstResFile,polymorphism, list_simulated, list_predicted,predicted_polymorphism_both_mapped, threshold)
    print_results(nb_predicted, list_simulated, polymorphism, threshold)
    if verbose:
        FP(list_predicted,polymorphism)
        FN(list_simulated,polymorphism)
